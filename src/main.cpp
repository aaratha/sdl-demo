...
/* we don't actually use any shaders in this one, so just give us lots of
 * options for backends. */
#define TESTGPU_SUPPORTED_FORMATS                             \
...
SDL_AppResult SDL_AppIterate(void* appstate) {
    ...
    if (!SDL_AcquireGPUSwapchainTexture(cmdbuf, state->windows[0],
    ...
    if (swapchainTexture != NULL) {
        ...
        color_target_info.clear_color.r =
            (float)(0.5 + 0.5 * SDL_sin(currentTime));
        color_target_info.clear_color.g =
            (float)(0.5 + 0.5 * SDL_sin(currentTime + SDL_PI_D * 2 / 3));
        color_target_info.clear_color.b =
            (float)(0.5 + 0.5 * SDL_sin(currentTime + SDL_PI_D * 4 / 3));
